( function($) {
    $(function() {
        ...
        $('#s-content-block').on("click", '.additional-colors-ajax', function(e) {
            e.preventDefault();
            var url = $(this).attr("data-url")
            $.get(url, function( data ){
                var dataPage = $(data).find(".s-product-page");
                var dataBreadCrumbs=$(data).find(".s-breadcrumbs-block");
                $(".s-product-page").html(dataPage);
                $(".s-breadcrumbs-block").html(dataBreadCrumbs);
            }).done(function(){
    ...
    $(document).ready( function() {
        updateHeaderCart = function(options) {
            var text = options["text"],
                count = options["count"],
                $cart = $("#cart-sum, [data-cart-sum]"),
                $cart_count = $('[data-cart-count]');

            if (count > 0) {
                $cart.html(text);
                $cart_count.html(count).removeClass('is-empty');
            } else {
                $cart.html($cart.data('empty'));
                $cart_count.html(0).addClass('is-empty');
            }
        };
...
var ProductPhotos = ( function($) {
    ...
    ProductPhotos.prototype.bindEvents = function() {
        ...
        that.$mainLink.on("click", function() {
            that.showSwipeBox( $(this).prop('title') );
            return false;
        });
    };
    ...
    ProductPhotos.prototype.showSwipeBox = function(title) {
        var that = this,
            before = [],
            after = [],
            images;
...
var ProductsFilter = ( function($) {
    ...
    ProductsFilter.prototype.bindEvents = function() {
        ...
        that.$form.find('[type="submit"]').hide();

        that.$form.find('.s-filter-header').click(function () {
            $(this).closest('.s-filter-group').toggleClass('collapsed');
        });
    ...
    ProductsFilter.prototype.onSubmit = function( $form ) {
        ...
        that.submit_timeout = setTimeout(function () {
            ...
            $.get(href, data, function(html) {
                ...
                if(!$form.find('.clear-filter').length) {
                    var a = '<a href="' + $form.prop('action') + '" class="s-button gray clear-filter">Очистить фильтры</a>';
                    $(a).appendTo($form.find('.s-buttons-wrapper'));
                }
            });
        }, 500);
    };
